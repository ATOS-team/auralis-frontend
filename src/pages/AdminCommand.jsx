...
const AdminStat = ({ label, value, trend, trendType, icon: Icon, path }) => (
    <Link to={path || '#'} className="glass-card p-6 md:p-8 rounded-[1.5rem] md:rounded-[2rem] clinical-shadow border-white/40 block hover:scale-[1.02] transition-transform">
        <div className="flex justify-between items-start mb-4 md:mb-6">
            ...
            {trend && (
                <div className={cn(
                    "flex items-center gap-1 px-2 py-1 md:px-3 rounded-full text-[10px] md:text-xs font-black",
        ...
        <div>
            <p className="text-[10px] md:text-sm font-black text-muted-foreground uppercase tracking-[0.2em] mb-1 md:mb-2">{label}</p>
            <h4 className="text-3xl md:text-4xl font-black text-foreground tracking-tighter">{value}</h4>
        </div>
    </Link>
);
...
const AdminCommand = () => {
    ...
    useEffect(() => {
        loadData();
        const intervalId = setInterval(loadData, 10000); // Auto-refresh every 10 seconds
        ...
        <div className="space-y-10 pb-20">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 md:gap-8">
                <div>
                    <div className="flex items-center gap-3 md:gap-4 mb-2 md:mb-3">
                        <div className="p-2 md:p-3 bg-indigo-900 text-white rounded-xl md:rounded-2xl shrink-0">
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 w-full md:w-auto">
                    <div className="relative flex-1">
                        ...
                        <input
                            ...
                            className="pl-12 pr-4 md:pr-6 py-3 md:py-4 w-full md:w-72 rounded-xl md:rounded-2xl border-2 border-slate-100 bg-slate-50 focus:bg-white focus:border-indigo-500 outline-none font-bold text-sm md:text-base"
                        />
                    </div>
                    <button className="p-3 md:p-4 bg-indigo-900 text-white rounded-xl md:rounded-2xl shadow-xl hover:scale-105 active:scale-95 transition-all flex justify-center shrink-0">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 md:gap-10">
                ...
                <div className="lg:col-span-2 space-y-8">
                    <div className="flex items-center justify-between">
                        <h3 className="text-xl md:text-2xl font-black tracking-tight flex items-center gap-2 md:gap-3">
                            <Activity className="h-6 w-6 md:h-7 md:w-7 text-indigo-900" /> Hospital Analytics
                        </h3>
                    </div>

                    {/* Analytics Grid */}
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
                        <div className="glass-card p-4 md:p-6 rounded-2xl md:rounded-[2rem] clinical-shadow border-white/40 bg-indigo-50/30">
                            <p className="text-[10px] md:text-xs font-black text-indigo-400 uppercase tracking-widest mb-1 md:mb-2">Clinical Load</p>
                            <h5 className="text-2xl md:text-3xl font-black text-indigo-900">{patients.length} <span className="text-sm md:text-2xl">Active</span></h5>
                            <p className="text-[9px] md:text-[10px] font-bold text-indigo-400 mt-1 md:mt-2 truncate">TOTAL REGISTERED</p>
                        </div>
                        <div className="glass-card p-4 md:p-6 rounded-2xl md:rounded-[2rem] clinical-shadow border-white/40 bg-rose-50/30 font-black">
                            <p className="text-[10px] md:text-xs font-black text-rose-400 uppercase tracking-widest mb-1 md:mb-2">Acuity Level</p>
                            <h5 className="text-2xl md:text-3xl font-black text-rose-900">{patients.filter(p => p.status === 'Critical').length} <span className="text-sm md:text-2xl">Critical</span></h5>
                            <p className="text-[9px] md:text-[10px] font-bold text-rose-400 mt-1 md:mt-2 truncate">HIGH RISK</p>
                        </div>
                        <div className="glass-card p-4 md:p-6 rounded-2xl md:rounded-[2rem] clinical-shadow border-white/40 bg-amber-50/30">
                            <p className="text-[10px] md:text-xs font-black text-amber-500 uppercase tracking-widest mb-1 md:mb-2">Avg. Age</p>
                            <h5 className="text-2xl md:text-3xl font-black text-amber-700">
                                {patients.length > 0
                                    ? Math.round(patients.reduce((acc, p) => acc + (parseInt(p.age) || 0), 0) / patients.length)
                                    : '--'} <span className="text-sm md:text-2xl">Yrs</span>
                            </h5>
                            <p className="text-[9px] md:text-[10px] font-bold text-amber-500 mt-1 md:mt-2 truncate">DEMOGRAPHICS</p>
                        </div>
                        <div className="glass-card p-4 md:p-6 rounded-2xl md:rounded-[2rem] clinical-shadow border-white/40 bg-emerald-50/30">
                            <p className="text-[10px] md:text-xs font-black text-emerald-400 uppercase tracking-widest mb-1 md:mb-2">Personnel</p>
                            <h5 className="text-2xl md:text-3xl font-black text-emerald-900">{allUsers.filter(u => u.role === 'Doctor').length} <span className="text-sm md:text-2xl">Staff</span></h5>
                            <p className="text-[9px] md:text-[10px] font-bold text-emerald-400 mt-1 md:mt-2 truncate">ACTIVE CLINICIANS</p>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between mt-8 md:mt-12">
                        <h3 className="text-xl md:text-2xl font-black tracking-tight flex items-center gap-2 md:gap-3">
                            <AlertCircle className="h-6 w-6 md:h-7 md:w-7 text-rose-600 animate-pulse" /> Critical Condition Monitor
                        </h3>
                    </div>

                    <div className="glass-card rounded-[1.5rem] md:rounded-[2.5rem] clinical-shadow border-rose-100 overflow-hidden bg-white/40">
                                    ...
                                    {patients.filter(p => p.status === 'Critical').map((patient) => (
                                        <tr key={patient.id} className="hover:bg-rose-50/30 transition-all">
                                            ...
                                            <td className="px-6 md:px-8 py-4 md:py-6 whitespace-nowrap">
                                                <Link to={`/patients/${patient.id}`} className="text-[10px] md:text-xs font-black text-indigo-600 underline tracking-tighter hover:text-indigo-900">
                                                    VIEW BIOMETRIC DATA
                                                </Link>
                                            </td>
                                        </tr>
                    ...
                    <div className="flex items-center justify-between mt-8 md:mt-12">
                        <h3 className="text-xl md:text-2xl font-black tracking-tight flex items-center gap-2 md:gap-3">
                            <Calendar className="h-6 w-6 md:h-7 md:w-7 text-indigo-600" /> Upcoming Clinical Sessions
                        </h3>
                    </div>

                    <div className="glass-card rounded-[1.5rem] md:rounded-[2.5rem] clinical-shadow border-white/40 overflow-hidden bg-white/40">
                        <div className="overflow-x-auto no-scrollbar">
                            <table className="w-full text-left">
                ...
                <div className="space-y-6 md:space-y-8">
                    <div className="glass-card p-6 md:p-8 rounded-[1.5rem] md:rounded-[2.5rem] border-white/40 clinical-shadow space-y-4 md:space-y-6">
                        <h4 className="text-base md:text-lg font-black uppercase tracking-widest text-indigo-900 border-b-2 border-slate-50 pb-3 md:pb-4 flex items-center justify-between">
                            Quick Controls <Settings className="h-4 w-4 md:h-5 md:w-5" />
                        </h4>
                    ...
                    <div className="glass-card p-6 md:p-8 rounded-[1.5rem] md:rounded-[2.5rem] bg-indigo-50/50 clinical-shadow border-indigo-100">
                        <h4 className="text-[10px] md:text-sm font-black uppercase tracking-[0.2em] text-indigo-900 mb-3 md:mb-4 flex items-center gap-2">
                            <CheckCircle2 className="h-4 w-4 md:h-5 md:w-5 text-emerald-500" /> System Status
                        </h4>
                        <p className="text-[10px] md:text-xs font-bold text-indigo-600 leading-relaxed">
